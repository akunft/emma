...
trait Trees { this: AST =>
  ...
  trait TreeAPI { this: API =>
    ...
    object Tree extends Node {
      ...
      // Predefined trees
      val Root  = Id(u.rootMirror.RootPackage)
      val Java  = Sel(Root, u.definitions.JavaLangPackage)
      val Scala = Sel(Root, u.definitions.ScalaPackage)
      ...
      object With {
        ...
        def apply[T <: Tree](tree: T)(
          ...
          if (noChange) tree else {
            ...
            if (sym != tree.symbol) setSymbol(tree, sym)
            if (tpe != tree.tpe) setType(tree, tpe)
            if (pos != tree.pos) atPos(pos)(tree)
            dup.asInstanceOf[T]
          }
        }
    ...
    private[ast] object Id extends Node {
      ...
      def apply(target: u.Symbol): u.Ident = {
        ...
        setSymbol(id, target)
        setType(id, target.info match {
          case u.NullaryMethodType(res) => res
          case tpe => tpe
        })
      }

      def unapply(id: u.Ident): Option[u.Symbol] =
        for (id <- Option(id) if has.sym(id)) yield id.symbol
    }
    ...
    private[ast] object Sel extends Node {
      ...
      def unapply(sel: u.Select): Option[(u.Tree, u.Symbol)] = sel match {
        case u.Select(target, _) if has.sym(sel) => Some(target, sel.symbol)
        case _ => None
      }
    }
    ...
    object Ref extends Node {
      ...
      def unapply(ref: u.Ident): Option[u.Symbol] = ref match {
        case Id(target) if !target.isMethod && !target.isPackage => Some(target)
        case _ => None
      }
    }
    ...
    object Acc extends Node {
      ...
      def apply(target: u.Tree, member: u.Symbol): u.Select = {
        ...
        assert(!member.isMethod,   s"$this member $member cannot be a method")
        Sel(target, member)
      }

      def unapply(acc: u.Select): Option[(u.Tree, u.Symbol)] = acc match {
        case Sel(Term(target), member) if !member.isStatic && !member.isMethod =>
          Some(target, member)
        case _ => None
      }
    }
